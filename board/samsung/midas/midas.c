...
void board_load_info(void)
{
	...
	/* Load serial number from eMMC CID */
	emmc = find_mmc_device(0);
	if (!emmc) {
...
/* Checks that battery is charged enough to allow boot */
...
{
	/* Set microUSB port back to serial console */
...
/* get boot mode based on key combo */
static enum boot_mode midas_check_keycombo(void)
{
	int ret = 0;

	/* the hw keys are active-low */
	...
	/* Power key pressed means that LPM should be skipped (i.e. this is a user initiated
	 * boot, even if we're plugged in).
...
/* get boot mode based on magic value in INFORM3 register */
...
/* power off board, indicating that it needs to be plugged in before it can boot */
...
/* Enable the USB network interface if booting into "console" mode */
...
int exynos_late_init(void)
{
	...
	/* magic value for boot mode in INFORM3 overrides key combo */
	...
	if (bat_state == BATTERY_ABORT) {
		...
		/* boot into low-power charging mode */
		env_set("bootmode", "lpm");
	}

	/*
	 * on the note 2: figure out which LCD is attached,
	 * and use the appropriate overlay when booting a kernel
	 */
	init_overlays();

	/*
	 * We use the notification LED in the absence of proper LCD support
	 * to indicate roughly what's going on -
	 * more of a "sign of life" than anything else.
	 */
	switch (mode) {
		case MODE_FASTBOOT:
